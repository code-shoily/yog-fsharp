...
let ``K5 Kuratowski witness is K5`` () =
    ...
    | Some witness -> Assert.Equal(K5, witness.Type)
    | None -> Assert.Fail("K5 should have a Kuratowski witness")
...
let ``K3,3 Kuratowski witness is K33`` () =
    ...
    | Some witness -> Assert.Equal(K33, witness.Type)
    | None -> Assert.Fail("K3,3 should have a Kuratowski witness")
...
let ``Tree decomposition validity check`` () =
    ...
    // Bags: 0 -> {1, 2}, 1 -> {2, 3}
    let bags = Map.ofList [ 0, Set.ofList [ 1; 2 ]; 1, Set.ofList [ 2; 3 ] ]
