...
let union (x: 'a) (y: 'a) (dsu: DisjointSet<'a>) : DisjointSet<'a> =
    ...
    if rootX = rootY then
        ...
        let rankX = dsu2.Ranks |> Map.tryFind rootX |> Option.defaultValue 0

        let rankY = dsu2.Ranks |> Map.tryFind rootY |> Option.defaultValue 0

        if rankX < rankY then
            { dsu2 with
                Parents = dsu2.Parents |> Map.add rootX rootY }
        else
            let dsu3 =
                { dsu2 with
                    Parents = dsu2.Parents |> Map.add rootY rootX }

            if rankX = rankY then
                { dsu3 with
                    Ranks = dsu3.Ranks |> Map.add rootX (rankX + 1) }
            else
                dsu3
...
let fromPairs (pairs: seq<'a * 'a>) : DisjointSet<'a> =
    pairs |> Seq.fold (fun dsu (x, y) -> union x y dsu) empty
