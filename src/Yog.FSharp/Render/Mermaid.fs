...
module Mermaid =
    ...
    let render (options: Options<'n, 'e>) (graph: Graph<'n, 'e>) : string =
        ...
        let hasHighlights =
            not (
                Set.isEmpty options.HighlightedNodes
        ...
        if hasHighlights then
            ...
            sb.AppendLine("  classDef highlightSource fill:#a8d8ea,stroke:#0288d1,stroke-width:3px")
            |> ignore

            sb.AppendLine("  classDef highlightSink fill:#f08080,stroke:#c62828,stroke-width:3px")
            |> ignore
    ...
    let renderMulti (options: MultiOptions<'n, 'e>) (graph: MultiGraph<'n, 'e>) : string =
        ...
        for kvp in graph.Edges do
            ...
            if graph.Kind = Directed || src <= dst then
                ...
                sb.AppendLine($"  {src} {arrow}|\"{options.EdgeLabel edgeId weight}\"| {dst}{style}")
                |> ignore
