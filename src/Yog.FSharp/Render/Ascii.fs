...
module Ascii =
    ...
    let private verticalWall (graph: Graph<'CellData, 'EdgeData>) rows cols r c =
        if c = 0 || c = cols then
            ...
            not (hasPassage graph (coordToId r (c - 1) cols) (coordToId r c cols))

    let private horizontalWall (graph: Graph<'CellData, 'EdgeData>) rows cols r c =
        if r = 0 || r = rows then
            true
        else
            not (hasPassage graph (coordToId (r - 1) c cols) (coordToId r c cols))
    ...
    let gridToString (grid: Grid<'CellData, 'EdgeData>) (occupants: Map<NodeId, string>) : string =
        if grid.Rows = 0 || grid.Cols = 0 then
            ...
            for row in 0 .. grid.Rows - 1 do
                sb.AppendLine(drawCellRow grid.Graph grid.Rows grid.Cols row occupants)
                |> ignore
    ...
    let gridToStringUnicode (grid: Grid<'CellData, 'EdgeData>) (occupants: Map<NodeId, string>) : string =
        if grid.Rows = 0 || grid.Cols = 0 then
            ...
            for r in 0 .. grid.Rows do
                sb.AppendLine(drawUnicodeIntersectionRow grid.Graph grid.Rows grid.Cols r)
                |> ignore

                if r < grid.Rows then
                    sb.AppendLine(drawUnicodeCellRow grid.Graph grid.Rows grid.Cols r occupants)
                    |> ignore
    ...
    let private addToroidalHints (ascii: string) rows cols =
        ...
        let bodyWithSides =
            lines
            |> Array.mapi (fun idx line -> if idx % 2 = 1 then $"> {line} <" else $"  {line}")
            |> String.concat "\n"
