...
module Maze =
    ...
    let private addPassage (grid: Grid<unit, int>) r1 c1 r2 c2 : Grid<unit, int> =
        ...
        { grid with Graph = addEdge id1 id2 1 grid.Graph }
    ...
    let private removePassage (grid: Grid<unit, int>) r1 c1 r2 c2 : Grid<unit, int> =
        let id1 = r1 * grid.Cols + c1
        let id2 = r2 * grid.Cols + c2
        { grid with Graph = removeEdge id1 id2 grid.Graph }

    /// Helper to get all neighbors of a cell on a 2D grid.
    let private getNeighbors (r, c) rows cols =
        let mutable neighbors = []
        if r > 0 then neighbors <- (r - 1, c) :: neighbors
        if r < rows - 1 then neighbors <- (r + 1, c) :: neighbors
        if c > 0 then neighbors <- (r, c - 1) :: neighbors
        if c < cols - 1 then neighbors <- (r, c + 1) :: neighbors
        neighbors
        ...
        if rows <= 0 || cols <= 0 then
            ...
            for r in 0 .. rows - 1 do
                ...
                for c in 0 .. cols - 1 do
                    ...
                    let shouldCloseRun =
                        if atNorthEdge then false
                        else atEastEnd || rng.NextDouble() > 0.5
                        
                    if r = rows - 1 && not atEastEnd then
                        grid <- addPassage grid r c r (c + 1)
                    elif r = rows - 1 && atEastEnd then
                        let runCol = if runStartCol = c then c else runStartCol + rng.Next(c - runStartCol + 1)
