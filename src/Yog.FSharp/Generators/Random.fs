...
module Random =
        ...
        if degrees.IsEmpty || degrees |> List.exists (fun d -> d < 0) || sum % 2 <> 0 then
            ...
            let rec attempt retries =
                if retries <= 0 then
                    ...
                    let stubsList =
                        List.init degrees.Length (fun idx -> List.replicate degrees.[idx] idx)
                        |> List.concat
                        |> List.toArray
                    ...
                    let pairs =
                        [ for i in 0..2 .. stubsList.Length - 2 -> (stubsList.[i], stubsList.[i + 1]) ]
                    ...
                    let valid =
                        (not hasSelfloop || allowSelfloops) && (not hasMultiedges || allowMultiedges)
    ...
    let rmat nNodes nEdges a b c d kind =
        if nNodes <= 0 || nEdges < 0 then
            ...
            let edges =
                [ for _ in 1..nEdges -> chooseEdgeRecursive 0 (nNodes - 1) 0 (nNodes - 1) ]
    ...
    let dcsbm n k (pIn: float) (pOut: float) degreeDist communitySizesOpt =
        ...
        let rawThetas =
            ...
            | PowerLaw gamma -> [| for i in 1..n -> Math.Pow(float i, -gamma) |]
            | Poisson -> [| for _ in 1..n -> 0.5 + rng.NextDouble() |]
            | Custom thetas ->
                if thetas.Length = n then
                    thetas
                else
                    [| for _ in 1..n -> 1.0 |]
        ...
        if n <= 0 || levels < 1 || branching < 2 then
            ...
            if baseLeafSize >= 1 then
                let probs =
                    ...
                    | None ->
                        match pMidOpt with
                        | Some pMid when levels = 2 -> [| pIn; pMid; pOut |]
                        | _ -> [| for l in 0..levels -> pIn + (pOut - pIn) * float l / float levels |]
    ...
    let powerLawGraph n gamma kMin kMaxOpt allowSelfloops allowMultiedges maxRetries =
        if n <= 0 then
            ...
            if kMin < 0 || kMax < kMin then
                ...
                let zeta =
                    Math.Pow(float kMin, 1.0 - gamma) - Math.Pow(float (kMax + 1), 1.0 - gamma)
