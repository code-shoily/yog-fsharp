...
module Model =
    ...
    let edgesBetween (graph: MultiGraph<'n, 'e>) from toNode =
        ...
        for eid in edgeIds do
            ...
            | Some(src, dst, data) when src = from && dst = toNode -> acc <- (eid, data) :: acc
    ...
    let successors id (graph: MultiGraph<'n, 'e>) =
        ...
        for eid in edgeIds do
            match Map.tryFind eid graph.Edges with
            | Some(src, dst, data) when src = id -> acc <- (dst, eid, data) :: acc
            | Some(src, dst, data) when dst = id && graph.Kind = Undirected -> acc <- (src, eid, data) :: acc
            | _ -> ()
    ...
    let predecessors id (graph: MultiGraph<'n, 'e>) =
        ...
        for eid in edgeIds do
            match Map.tryFind eid graph.Edges with
            | Some(src, dst, data) when dst = id -> acc <- (src, eid, data) :: acc
            | Some(src, dst, data) when src = id && graph.Kind = Undirected -> acc <- (dst, eid, data) :: acc
            | _ -> ()
    ...
    let private countSelfLoops (graph: MultiGraph<'n, 'e>) id =
        ...
        for eid in edgeIds do
            match Map.tryFind eid graph.Edges with
            | Some(src, dst, _) when src = id && dst = id -> count <- count + 1
            | _ -> ()
    ...
    let outDegree id (graph: MultiGraph<'n, 'e>) =
        let baseCount =
            Map.tryFind id graph.OutEdgeIds |> Option.map Set.count |> Option.defaultValue 0
    ...
    let inDegree id (graph: MultiGraph<'n, 'e>) =
        let baseCount =
            Map.tryFind id graph.InEdgeIds |> Option.map Set.count |> Option.defaultValue 0
...
module Traversal =
    ...
    let foldWalk
        (source: NodeId)
        (initial: 'acc)
        (folder: 'acc -> NodeId -> WalkMetadata -> WalkControl * 'acc)
        (graph: MultiGraph<'n, 'e>)
        : 'acc =
        let queue =
            System.Collections.Generic.Queue<NodeId * NodeId option * EdgeId option>()
        ...
        while queue.Count > 0 && not halted do
            ...
            let meta =
                { Depth = depth
                  Parent = Option.map2 (fun p e -> (p, e)) parent edgeId }
...
module Eulerian =
    ...
    let private isConnected (graph: MultiGraph<'n, 'e>) =
        let nonIsolated =
            graph.Nodes.Keys |> Seq.filter (fun n -> Model.degree n graph > 0) |> Seq.toList
    ...
    let hasEulerianCircuit (graph: MultiGraph<'n, 'e>) =
        if graph.Nodes.Count = 0 || Model.size graph = 0 then
            ...
            | Directed ->
                let allBalanced =
                    graph.Nodes.Keys
                    |> Seq.forall (fun n -> Model.inDegree n graph = Model.outDegree n graph)
    ...
    let private firstNonIsolated (graph: MultiGraph<'n, 'e>) =
        graph.Nodes.Keys |> Seq.tryFind (fun n -> Model.degree n graph > 0)
