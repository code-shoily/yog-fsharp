...
let isValid (td: TreeDecomposition<'tn, 'te>) (graph: Graph<'n, 'e>) : bool =
    ...
    let edgeCoverage =
        if vertexCoverage then
            ...
            allGraphEdges |> List.forall (fun (u, v) ->
                td.Bags |> Map.exists (fun _ bag -> bag.Contains(u) && bag.Contains(v))
            )
        else
            false
    ...
    let runningIntersection =
        if edgeCoverage then
            allNodes graph |> List.forall (fun v ->
                let containing = 
                    bagIndices 
                    |> List.filter (fun idx -> td.Bags.[idx].Contains(v))
