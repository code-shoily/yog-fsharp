...
module Coloring =
    ...
    let coloringDsatur (graph: Graph<'n, 'e>) : ColoringResult =
        ...
        if List.isEmpty nodes then
            ...
            let adj = nodes |> List.map (fun node -> node, Set.ofList (neighborIds node graph)) |> Map.ofList
            let degrees = nodes |> List.map (fun node -> node, (neighbors node graph).Length) |> Map.ofList
            ...
            while uncolored.Count > 0 do
                let node =
                    uncolored
                    |> Seq.maxBy (fun n -> saturations.[n], degrees.[n])
    ...
    let rec private exactBacktrack (nodes: NodeId list) (coloring: Map<NodeId, int>) (maxUsed: int) (state: ExactColoringState) : unit =
