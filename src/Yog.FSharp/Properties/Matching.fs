...
let hopcroftKarp (graph: Graph<'n, 'e>) : Map<NodeId, NodeId> =
    ...
    | Some part ->
        ...
        for u in uNodes do
            match pairU.[u] with
            | Some v -> matching <- matching |> Map.add u v |> Map.add v u
            | None -> ()
                        ...
                        if isDummyU || isDummyV then
                            ...
                            | None ->
                                ...
                                | None ->
                                    invalidArg
                                        "graph"
                                        "hungarian requires a complete bipartite graph (edges between all L and R nodes)"
...
let blossomMaximumMatching (graph: Graph<'n, 'e>) : Map<NodeId, NodeId> =
    ...
    if nodes.IsEmpty then
        ...
        let contract
            ...
            (v: NodeId)
            (b: NodeId)
            (parent: Dictionary<NodeId, NodeId option>)
            (queue: Queue<NodeId>)
            (inQueue: HashSet<NodeId>)
            =
