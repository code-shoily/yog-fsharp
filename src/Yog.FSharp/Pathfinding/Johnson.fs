...
module Johnson =
    ...
    let johnson
        ...
        let tempSource =
            if List.isEmpty allNodesList then 0
            else (allNodesList |> List.max) + 1
        ...
        let regularEdges =
            allNodesList
            |> List.collect (fun u ->
                successors u graph |> List.map (fun (v, w) -> (u, v, w))
            )
        ...
        if hasNegativeCycle then
            ...
            let potentials =
                ...
                |> List.map (fun v ->
                    ...
                    if distances.TryGetValue(v, &dist) then
                        v, dist
                    else
                        v, zero
                )
    ...
    let johnsonInt (graph: Graph<'n, int>) : Result<Map<NodeId * NodeId, int>, unit> =
        johnson 0 (+) (-) compare graph
