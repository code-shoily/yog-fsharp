...
module Yen =
            ...
            for i in 0 .. maxSpurIndex do
                ...
                | Some spurPath ->
                    ...
                    if not (seenPaths.Contains totalNodes) && not (seenCandidates.Contains totalNodes) then
                        let newPath = { Nodes = totalNodes; TotalWeight = totalWeight }
                ...
                while remaining > 0 && not stop do
                    if candidatesPq.Count = 0 then
                        ...
                        if not (seenPaths.Contains candidate.Nodes) then
                            ...
                            generateCandidates graph candidate !paths seenPaths seenCandidates candidatesPq goal zero add compare
    ...
    let kShortestPathsFloat (start: NodeId) (goal: NodeId) (k: int) (graph: Graph<'n, float>) : Path<float> list option =
