...
let floydWarshall
    ...
    for i in nodes do
        for j in nodes do
            if i = j then
                ...
                match graph.OutEdges |> Map.tryFind i |> Option.bind (Map.tryFind j) with
    ...
    for i in nodes do
        ...
        if distances.TryGetValue((i, i), &selfDist) && compare selfDist zero < 0 then
    ...
    if hasNegativeCycle then
        ...
        let mapSeq = distances |> Seq.map (fun kvp -> kvp.Key, kvp.Value)
