...
let serializeWith
    ...
    let nodeKeys =
        graph.Nodes
        |> Map.toSeq
        |> Seq.collect (fun (_, d) -> nodeAttr d)
        |> Seq.map fst
        |> Seq.distinct
    ...
    let edgeKeys =
        graph.OutEdges
        |> Map.toSeq
        |> Seq.collect (fun (_, m) -> m |> Map.toSeq |> Seq.collect (fun (_, d) -> edgeAttr d))
        |> Seq.map fst
        |> Seq.distinct
    ...
    let nodes =
        ...
        |> Seq.map (fun (id, data) ->
            ...
            nodeAttr data
            |> List.iter (fun (k, v) -> el.Add(XElement(ns + "data", XAttribute(XName.Get "key", k), v)))
...
///
let deserialize (xml: string) : Graph<Map<string, string>, Map<string, string>> = deserializeWith id id xml
...
///
let readFile (path: string) : Graph<Map<string, string>, Map<string, string>> = File.ReadAllText(path) |> deserialize
