...
let render (options: Options<'n, 'e>) (graph: Graph<'n, 'e>) : string =
    ...
    let header =
        if graph.Kind = Directed then "graph TD"
        else "graph LR"
    ...
    for kvp in graph.Nodes do
        ...
        let style =
            if options.HighlightedNodes.Contains(id) then ":::highlight"
            else ""

        sb.AppendLine($"  {id}[\"{label}\"]{style}")
        |> ignore
    ...
    for KeyValue (src, targets) in graph.OutEdges do
        for KeyValue (dst, weight) in targets do
            if graph.Kind = Directed || src <= dst then
                ...
                let style =
                    if isHighlighted then ":::highlightEdge"
                    else ""
