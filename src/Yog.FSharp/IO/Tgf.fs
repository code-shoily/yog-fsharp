...
let parse (graphType: GraphType) (nodeParser: string -> 'n) (edgeParser: string option -> 'e) (input: string) : Result<Graph<'n, 'e>, string> =
        ...
        for line in nodeLines do
            ...
            if trimmed <> "" && not (trimmed.StartsWith("#")) then
                ...
                | [| idStr |] ->
                    ...
                    | true, id ->
                        graph <- addNode id (nodeParser (id.ToString())) graph
                    | false, _ ->
                        possible <- false
                ...
                | [| idStr; label |] ->
                    match Int32.TryParse(idStr) with
                    | true, id ->
                        graph <- addNode id (nodeParser label) graph
                    | false, _ ->
                        possible <- false
                        errorMsg <- sprintf "Invalid node ID: %s" idStr
                | _ -> ()
