...
let mapEdges (f: 'e -> 'f) (graph: Graph<'n, 'e>) : Graph<'n, 'f> =
    let mapOuter outerMap =
        outerMap
        |> Map.map (fun _ innerMap -> innerMap |> Map.map (fun _ -> f))
...
let filterNodes (predicate: 'n -> bool) (graph: Graph<'n, 'e>) : Graph<'n, 'e> =
    ...
    let pruneEdges outerMap =
        ...
        |> Map.map (fun _ innerMap ->
            innerMap
            |> Map.filter (fun dst _ -> Set.contains dst keptIds))
                ...
                (fun acc dst ->
                    if src = dst then
                        ...
                        if hasEdge then
                            ...
                            addEdge src dst defaultWeight acc)
                g)
        initGraph
...
let subgraph (ids: NodeId list) (graph: Graph<'n, 'e>) : Graph<'n, 'e> =
    ...
    let keptNodes =
        graph.Nodes
        |> Map.filter (fun id _ -> Set.contains id idSet)
    ...
    let prune outerMap =
        ...
        |> Map.map (fun _ innerMap ->
            innerMap
            |> Map.filter (fun dst _ -> Set.contains dst idSet))
        |> Map.filter (fun _ innerMap -> not innerMap.IsEmpty)
