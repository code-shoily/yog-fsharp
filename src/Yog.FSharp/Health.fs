...
module Health =
    ...
    let eccentricity
        ...
        if numNodes <= 1 then
            ...
            let distances =
                Yog.Pathfinding.Dijkstra.singleSourceDistances zero add compare node graph
    ...
    let diameter (zero: 'e) (add: 'e -> 'e -> 'e) (compare: 'e -> 'e -> int) (graph: Graph<'n, 'e>) : 'e option =
    ...
    /// Returns None if the graph is disconnected or empty.
    let radius (zero: 'e) (add: 'e -> 'e -> 'e) (compare: 'e -> 'e -> int) (graph: Graph<'n, 'e>) : 'e option =
        let nodes = allNodes graph
        let n = nodes.Length
    ...
    let assortativity (graph: Graph<'n, 'e>) : float =
        ...
        let degrees =
            nodes
            |> List.map (fun node -> node, (successors node graph).Length)
            |> Map.ofList
    ...
    let averagePathLength
        ...
        if numNodes <= 1 then
            ...
            while i < numNodes && not disconnected do
                ...
                let distances =
                    Yog.Pathfinding.Dijkstra.singleSourceDistances zero add compare u graph
