...
let hits (maxIterations: int option) (tolerance: float option) (graph: Graph<'n, 'e>) : HitsResult =
    ...
    if n = 0 then
        ...
        let inMap =
            match graph.Kind with
            | Undirected -> nodes |> List.map (fun node -> node, successorIds node graph) |> Map.ofList
            | Directed -> nodes |> List.map (fun node -> node, predecessorIds node graph) |> Map.ofList
        ...
        while iter < maxIter && not converged do
            ...
            for node in nodes do
                ...
                let score =
                    preds
                    |> List.fold (fun sum pred -> sum + (Map.tryFind pred hub |> Option.defaultValue 0.0)) 0.0
            ...
            for node in nodes do
                ...
                let score =
                    succs
                    |> List.fold (fun sum succ -> sum + (Map.tryFind succ newAuth |> Option.defaultValue 0.0)) 0.0
            ...
            let normalizedAuth =
                if normAuth > 0.0 then
                    newAuth |> Map.map (fun _ v -> v / normAuth)
                else
                    newAuth

            let normalizedHub =
                if normHub > 0.0 then
                    newHub |> Map.map (fun _ v -> v / normHub)
                else
                    newHub
