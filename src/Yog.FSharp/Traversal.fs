...
let private doTopologicalSort (graph: Graph<'n, 'e>) =
    ...
    inDegrees
    |> Map.iter (fun id degree -> if degree = 0 then queue.Enqueue(id))
...
let implicitDijkstra
    ...
    while frontier.Count > 0 && not halted do
        ...
        if
            best.TryGetValue(node, &currentBest)
            && cost <= currentBest
        then
            ...
            | Continue ->
                for (neighbor, edgeCost) in successors node do
                    ...
                    if
                        not (best.TryGetValue(neighbor, &neighborBest))
                        || newCost < neighborBest
                    then
